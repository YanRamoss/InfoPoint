export default async function CardapioComp() {
  type InfoItem = {
    prato: string;
    dia: string; // Adjusted to use `dia` based on your previous context
  };

  const response = await fetch('https://infopoint-sigma.vercel.app/api/cardapio', { next: { revalidate: 10 }, method: "GET" });
  const data: InfoItem[] = await response.json(); // Specify that data is an array of InfoItem

  return (
    <div className="max-w-4xl mx-auto min-h-[600px]">
      <h1 className="text-center text-[25px] py-[30px]">Cardápio Semanal</h1>
      <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-5">
        {data.map((item: InfoItem, index: number) => ( // Specify the type for item
